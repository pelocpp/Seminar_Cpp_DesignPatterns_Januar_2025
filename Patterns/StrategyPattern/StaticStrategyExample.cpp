...
namespace StaticStrategyPatternExample {
    ...
    {
        { list.start (oss) } -> std::same_as<void>;  // std::same_as ist ebenfalls ein Concept
    ...
    {
    private:
        TList              m_list;  // m_list: start, end, add_item
        std::ostringstream m_oss;
    ...
    public:
        void append_list(std::initializer_list<std::string> items)
        {
            ...
            m_list.end(m_oss);   // '.' ==> DIREKTER AUFRUF
        }

        std::string str() const { return m_oss.str(); }
    };

    struct MarkdownListStrategyEx // : keine Basisklasse: Vorteil
    {
    ...
    struct HtmlListStrategyEx // : keine Basisklasse: Vorteil
    {
        void start(std::ostringstream& oss) {
            oss << "<ul>" << std::endl;
        }
